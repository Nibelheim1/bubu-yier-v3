...
async def main():
 ...
 async with async_playwright() as p:
  ...
  await check('twelve scene restarts do not accumulate scene input handlers',r'''()=>{const d=__BUBU_TEST__,counts=[];for(let n=0;n<12;n++){d.go(n%3+1,'bubu');d.fastStep(3);counts.push(d.scene.input.listenerCount('pointerupoutside'));}return {counts,keys:d.app.input.keys.size,pointers:d.app.input.pointers.size};}''',lambda r:all(c==1 for c in r['counts']) and r['keys']==0 and r['pointers']==0)
